...
log(
  "Folder detail: name shown",
  folderHeader?.includes("Strata Documents"),
);
...
log("Directory: all columns",
  headers.includes("Name") && headers.includes("Email") &&
  headers.includes("Children") && headers.includes("Dogs"),
  headers.join(", "));
...
await page.waitForTimeout(300);
const noResults = await page.locator("tbody").getByText("No members match").isVisible();
...
log("Profile: heading", (await page.locator("h1").textContent())?.includes("Profile"));

// name is read-only text, not an input
const nameText = page.locator("p").filter({ hasText: /Test|Spencer/ }).first();
log("Profile: name shown as text", await nameText.isVisible());

log("Profile: 'Account Settings' link", await page.getByText("Account Settings").isVisible());

log("Profile: phone input editable", await page.locator("#phone").isEnabled());
log("Profile: lot input editable", await page.locator("#lot_number").isEnabled());
...
log("Profile: visibility toggle", await page.getByText("Show in member directory").first().isVisible());
log("Profile: Save button", await page.getByRole("button", { name: /Save/i }).isVisible());
...
log("Admin: heading", (await page.locator("h1").textContent())?.includes("Members"));

const activeTab = page.locator('a[class*="font-medium"]').filter({ hasText: "Members" });
...
log("Admin: Active badge", await page.locator("tbody").getByText("Active").first().isVisible());

// kebab menu
const firstRow = page.locator("tbody tr").first();
await firstRow.locator("button").last().click({ force: true });
await page.waitForTimeout(300);
log("Admin: Edit option", await page.getByRole("menuitem", { name: "Edit" }).isVisible());
log("Admin: Deactivate option", await page.getByRole("menuitem", { name: "Deactivate" }).isVisible());
await page.keyboard.press("Escape");

// expand add form
await addMemberBtn.click();
await page.waitForTimeout(300);
log("Admin: form expands", await page.locator('input[name="first_name"]').isVisible());
log("Admin: role select", await page.locator('[data-slot="select-trigger"]').isVisible());
...
await unauthPage.goto("http://localhost:3000/sign-up", { waitUntil: "networkidle" });
log("Public: /sign-up → invitation required", await unauthPage.getByText("Invitation required").isVisible());

for (const path of ["/documents", "/directory", "/profile", "/admin/members"]) {
  await unauthPage.goto(`http://localhost:3000${path}`, { waitUntil: "networkidle" });
  log(`Public: ${path} → sign-in redirect`, unauthPage.url().includes("/sign-in"));
}
