...
export const getCurrentAppUser = cache(async (): Promise<AppUser | null> => {
  ...
  const { data: byClerk } = await supabaseAdmin
    .from("app_user")
    .select(
      "id, clerk_id, email, first_name, last_name, role_id, active, accepted_at, onboarded_at, profile_confirmed_at, avatar_url",
    )
  ...
  const { data: byEmail } = await supabaseAdmin
    .from("app_user")
    .select(
      "id, clerk_id, email, first_name, last_name, role_id, active, accepted_at, onboarded_at, profile_confirmed_at",
    )
    .ilike("email", email)
    .maybeSingle();
