...
2. **Supabase → Resend (server action):** When the primary admin sends a group email, a server action resolves group membership from Supabase, then sends the email via Resend's batch API. Delivery events flow back via Resend webhooks to update the email log.
...
Roles are admin-configurable — the primary admin can create new roles and assign capabilities through the admin UI.
