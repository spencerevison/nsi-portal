...
- Context switching for the primary admin: managing a document means leaving the portal, opening the admin app, making the change, then returning to verify
...
- Adds another service, another login, and another tool for the primary admin to learn
- Limited customization for NSI-specific workflows (invitation flow, group email compose, document folder management)
- Disconnected from the portal — the primary admin manages content in one tool and views the result in another
...
- Admin UI is optimized for simplicity and the primary admin's specific workflows, not for general-purpose data management — complex database operations (bulk imports, schema changes) will be done through Supabase Studio or CLI
